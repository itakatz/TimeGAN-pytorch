...
class BaseModel():
  ...
  def train(self, log_batch = False):
    ...
    for epoch in range(self.opt.num_epochs):
        ...
        num_to_generate = 8
        generated_samples = self.generation(num_to_generate)
        k = 0 # which channel to plot (the auto-encoder decodes all channels - TODO consider just decode 1 channel??)
        for ifig in range(num_to_generate):
            fig, ax = plt.subplots()
            ax.plot(generated_samples[ifig,:,k], 'o')
            #ax.legend(['x','x_tilde'])
            ax.grid()
            self.writer.add_figure(f'Generator/{ifig}', fig, epoch)

        self.writer.flush()
        print(f'Joint training epoch: {epoch}/{self.opt.num_epochs} training loss gen {running_loss_g:.5f}, disc {running_loss_d:.5f}')
